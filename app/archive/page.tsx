...
type Page = "home" | "method" | "participants" | "participant" | "notice" | "team" | "staff" | "revision" | "materials" | "publications" | "systems" | "activity" | "reference" | "ethics" | "memos" | "memo";
...
export default function RecoveredWebsite() {
  ...
  const sync = () => {
    const completed = ([1, 2, 3, 4, 5, 6, 7, 8] as TaskNumber[]).filter((task) => localStorage.getItem(solvedKey(task)) === "true").length;
    const task: TaskNumber = localStorage.getItem(solvedKey(1)) !== "true" ? 1 : localStorage.getItem(solvedKey(2)) !== "true" ? 2 : localStorage.getItem(solvedKey(3)) !== "true" ? 3 : localStorage.getItem(solvedKey(4)) !== "true" ? 4 : localStorage.getItem(solvedKey(5)) !== "true" ? 5 : localStorage.getItem(solvedKey(6)) !== "true" ? 6 : localStorage.getItem(solvedKey(7)) !== "true" ? 7 : 8;
  ...
  const sectionIsActive = (section: Page) => page === section || (section === "participants" && page === "participant") || (section === "team" && (page === "staff" || page === "revision")) || (section === "memos" && page === "memo") || (section === "systems" && (page === "activity" || page === "reference"));
  ...
  return <main className="legacy-site"><div className="legacy-shell">
    ...
    <div className="legacy-columns"><aside><button className="sidebar-title" onClick={() => go("home")}>Project Morrowfield</button>{(["home", "team", "materials", "publications", "ethics", "memos", "systems"] as Page[]).map((id) => <button key={id} className={sectionIsActive(id) ? "active" : ""} onClick={() => go(id)}>{id === "home" ? "Overview" : labels[id]}</button>)}<hr/><small>Protocol BWU-03-118<br/>Last updated {storyStage >= 1 ? "09/19/03" : "09/18/03"}</small></aside>
      <article><p className="breadcrumbs">Bellwether › Research › {labels[page]}</p>
        ...
        {page === "team" && <><h1>Research Team</h1><p className="lead">Project Morrowfield was administered by the Memory and Suggestibility Laboratory.</p><div className="staff-list"><section><h2>Dr. Elian Voss</h2><p>Principal investigator · Experimental design and participant interviews</p></section><section><h2>Dr. Miriam Calder</h2><p>Faculty sponsor · Research ethics and methodology review</p></section><section><h2>Jonas Reed</h2><p>Graduate researcher · Materials construction and data coding</p></section><section><h2>Nadia Bell</h2><p>Research assistant · Session scheduling and records management</p></section>{storyStage >= 2 && <section><h2>{storyStage >= 7 ? <button className="log-record-link" onClick={() => go("staff")}>Eleanor Harrow</button> : "Eleanor Harrow"}</h2><p>Project archivist · Collection preparation and records maintenance</p></section>}</div><p className="legacy-meta">Laboratory roster · Autumn 2003</p></>}
        {page === "staff" && storyStage >= 7 && <><button className="record-back" onClick={() => go("team")}>‹ Return to Research Team</button><h1>Eleanor Harrow</h1><p className="lead">Project archivist · Memory and Suggestibility Laboratory</p><dl className="record-metadata"><div><dt>Display name</dt><dd>Eleanor Harrow</dd></div><div><dt>Project role</dt><dd>Collection preparation and records maintenance</dd></div><div><dt>Personnel directory</dt><dd><Fact id="harrow-directory" label="personnel directory result" task={8}>{storyStage >= 8 ? "Personnel record restricted" : "No employee record located"}</Fact></dd></div><div><dt>Profile history</dt><dd><button className="log-record-link" onClick={() => go("revision")}>View revision record</button></dd></div></dl><h2>Attributed project records</h2><p><button className="log-record-link" onClick={() => { setSelectedLogId("01-A"); go("participant"); }}>Participant Log 01-A</button> · Initial guided recall · Participant 014</p><p>Morrowfield Methodology Appendix · records preparation credit</p><p>Constructed environments and autobiographical conformity · acknowledgements</p><p className="legacy-meta">Department profile index · record EH-118</p></>}
        {page === "revision" && storyStage >= 7 && <><button className="record-back" onClick={() => go("staff")}>‹ Return to Eleanor Harrow</button><h1>Profile Revision History</h1><p className="lead">Department profile index · record EH-118</p><table className="log-table"><thead><tr><th>Revision</th><th>Date</th><th>Account</th><th>Action</th></tr></thead><tbody><tr><td>01</td><td><Fact id="harrow-profile-created" label="profile creation record" task={8}>October 11, 2003 · archive-maint</Fact></td><td>archive-maint</td><td>created</td></tr><tr><td>02</td><td>October 11, 2003</td><td>system</td><td>indexed</td></tr></tbody></table><p className="legacy-meta">No earlier profile revision is present in the recovered index.</p></>}
        {page === "materials" && <><h1>Study Materials</h1><p>Participants were shown a constructed municipal archive representing the fictional town of Morrowfield.</p><ul className="materials-list"><li><strong>Packet A</strong><span>Municipal history and founding records</span></li><li><strong>Packet B</strong><span>Street map and civic landmarks · {storyStage >= 4 ? "original issue 09/03/03" : <Fact id="packet-b-revision" label="Packet B revision record" task={4}>revised 09/22/03</Fact>}</span></li><li><strong>Packet C</strong><span>Festival photographs, 1971-1987</span></li><li><strong>Packet D</strong><span>Simulated newspaper extracts</span></li></ul><h2>Technical documents</h2><p><a className="document-link" href="/documents/morrowfield-methodology.pdf" target="_blank" rel="noopener" onClick={() => capture("methodology-pdf", "methodology appendix", 2)}>Morrowfield methodology appendix (PDF, 312 KB)</a></p><p className="legacy-meta">Materials register copy 4 · Checked against laboratory inventory</p></>}
        ...
        {page === "activity" && !(storyStage >= 3 && selectedActivity[0] === "October 11, 2003") && <><button className="record-back" onClick={() => go("systems")}>‹ Return to Access and Export Register</button><h1>Server Activity Record</h1><p className="lead">Recovered HTTP and collection activity entry.</p><dl className="record-metadata"><div><dt>Authenticated record</dt><dd>{isRestoredActivity ? <><Fact id="current-access-date" label="restored access date" task={7}>{selectedActivity[0]}</Fact> · {selectedActivity[1]}</> : selectedActivity[0] === "October 11, 2003" ? <Fact id="access-log" label="final authenticated access" task={3}>{selectedActivity[0]} · {selectedActivity[1]}</Fact> : `${selectedActivity[0]} · ${selectedActivity[1]}`}</dd></div><div><dt>Resource</dt><dd>{isRestoredActivity && storyStage === 6 ? <button className="log-record-link" onClick={() => go("reference")}>{selectedActivity[2]}</button> : selectedActivity[0] === "October 11, 2003" ? <Fact id="export-file" label="post-closure export" task={3}>{selectedActivity[2]}</Fact> : selectedActivity[2]}</dd></div><div><dt>Action</dt><dd>{selectedActivity[3]}</dd></div><div><dt>Source</dt><dd>morrowfield-access.log</dd></div></dl><h2>Register notes</h2><p>This entry was reconstructed from the server activity register. Export events are retained separately from the corresponding collection files.</p><p className="legacy-meta">Automated record · University local time</p></>}
        {page === "reference" && storyStage >= 6 && <><button className="record-back" onClick={() => go("activity")}>‹ Return to Server Activity Record</button><h1>Resource Reference</h1><p className="lead">Reference record retained with the server access register.</p><dl className="record-metadata"><div><dt>Reference</dt><dd>restored-export.ref</dd></div><div><dt>Request</dt><dd>accession-session-041</dd></div><div><dt>Accession</dt><dd><Fact id="current-case" label="linked accession case" task={7}>Case 27-041</Fact></dd></div><div><dt>Resolved resource</dt><dd><Fact id="reopened-export" label="resolved export file" task={7}>participant-index-03.zip</Fact></dd></div><div><dt>Status</dt><dd>File contents unavailable</dd></div></dl><p className="legacy-meta">Reference RX-03 · Automated reconstruction</p></>}
      </article>
