...
export default function RecoveredWebsite() {
  ...
  const sync = () => {
    const completed = ([1, 2, 3, 4, 5, 6] as TaskNumber[]).filter((task) => localStorage.getItem(solvedKey(task)) === "true").length;
  ...
  const selectedActivity = serverLogs[selectedActivityIndex] ?? serverLogs[0];
